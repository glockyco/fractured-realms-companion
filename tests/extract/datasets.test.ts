...
function bundle(): string {
  ...
  const actionLists = skills.map((skill, i) => `${skill}:[{id:"${skill === 'woodcutting' ? 'chop_test' : skill === 'archaeology' ? 'dig_ancient_cairn' : `act_${i}`}\",levelReq:${skill === 'archaeology' ? 38 : 1},xp:2,interval:100,outputs:{ancient_spore:1}}]`).join(',');
  ...
  return `/* fake { } */ const ITEMS={${items}}; const ACTIONS={${actionLists}}; const SKILLS=[{id:"hitpoints",name:"Hitpoints"},${skillList}]; const BUILDINGS=${buildings}; const DIGS=${digsites}; const STRINGS=${strings}; function xp(){ const t=3; const ignored=\`template { \${{x:1}} }\`; Math.floor(t/3.5); return Array.from({length:100},(_,i)=>i<2?0:i*10); }`;
}
