...
import { listTables, buildSheet, toCell, buildAllSheets } from '../../scripts/export-sheets.mjs';
...
test('CLI prints an offline summary from the state model.db', { skip: !HAS_SQLITE }, () => {
  const dir = mkdtempSync(join(tmpdir(), 'frc-sheets-'));
  try {
    ...
    const result = spawnSync(process.execPath, ['scripts/export-sheets.mjs'], {
      encoding: 'utf8',
