...
- **Items:** search by item name or ID; view descriptions, values, healing data, icons, deterministic and rare sources, and uses.
- **Skills:** browse extracted skill actions and their requirements, intervals, inputs, outputs, and locations.
- **Planner:** request an item quantity from the current inventory/equipment snapshot. The planner resolves deterministic input dependencies in order, applies skill-level gates, reports missing tools, detects cycles, and explains no-source and rare-only cases.
...
- The planner uses deterministic outputs only. Rare drops are shown as information and can produce a `rare-only` block; missing tools produce warnings rather than automatic tool-crafting.
- Game restrictions such as combat, dungeons, raids, or a full bag can refuse an action. The executor surfaces the refusal and never bypasses those game checks.
