...
function createApplication(shell, modelJson, api) {
  ...
  // Single funnel for every queue-goal edit (add, reorder, remove, clear). `apply`
  // mutates state.queueGoals synchronously and returns an optional live-region
  // announcement; the edit is then persisted, re-planned OFF-THREAD via afterPlan,
  // and re-rendered (with optional post-render focus restoration). Routing all edits
  // here guarantees none freezes the UI by resolving the queue synchronously on the
  // main thread, matching how the start button already plans through afterPlan.
  const commitQueueEdit = (apply, afterRender) => {
    const announcement = apply();
    persistQueue();
    if (announcement) announcePlan(announcement);
    return afterPlan(() => { renderPlan(); afterRender?.(); });
  };
  ...
  planForm.addEventListener('submit', (event) => { event.preventDefault(); try { const target = makeTarget(); setFormError(); const entry = { id: `plan-${state.nextPlanId++}`, target }; setComposerOpen(false); commitQueueEdit(() => { state.queueGoals.push(entry); return `Added ${targetLabel(target)} as priority ${state.queueGoals.length}.`; }); } catch (error) { const message = error instanceof Error ? error.message : String(error); setFormError(message, message.includes('Choose an item') ? planItem : planQty); } });
  ...
  planPanel.addEventListener('click', (event) => { if (isExecutionLocked(state.executorStatus?.phase)) return; const move = event.target.closest?.('[data-queue-move]'); if (move) { const dir = move.dataset.queueMove; const goalId = move.dataset.queueGoal; const from = state.queueGoals.findIndex((goal) => goal.id === goalId); if (from < 0) return; const to = dir === 'top' ? 0 : dir === 'up' ? from - 1 : from + 1; if (to < 0 || to >= state.queueGoals.length) return; commitQueueEdit(() => { const [moved] = state.queueGoals.splice(from, 1); state.queueGoals.splice(to, 0, moved); return `Moved ${targetLabel(moved.target)} to priority ${to + 1}.`; }, () => { planPanel.querySelector(`[data-queue-move="${dir}"][data-queue-goal="${goalId}"]`)?.focus?.(); }); return; } const remove = event.target.closest?.('[data-queue-remove]'); if (!remove) return; const index = state.queueGoals.findIndex((goal) => goal.id === remove.dataset.queueRemove); if (index < 0) return; commitQueueEdit(() => { const [removed] = state.queueGoals.splice(index, 1); return `Removed ${targetLabel(removed.target)}.`; }, () => { const next = state.queueGoals[index] || state.queueGoals[index - 1]; planPanel.querySelector(next ? `[data-queue-remove="${next.id}"]` : '#fr-plan-compose-toggle')?.focus?.(); }); });
