...
function writeRows(db: NonNullable<ReturnType<typeof openDatabase>>, model: GameModel): void {
  ...
  for (const [itemId, value] of Object.entries(model.potions)) {
    const row = asRecord(value);
    insert(db, 'potions', ['item_id', 'slot', 'mult', 'dur_ms', 'cd_ms', 'family', 'data_json'], [itemId, scalar(row.slot), scalar(row.mult), scalar(row.durMs), scalar(row.cdMs), scalar(row.family), json(row)]);
  }
